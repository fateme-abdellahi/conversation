...
class ConversationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if request.data.get('message') is None or request.data.get('message').strip() == "":
        ...
        if conversation_serializer.is_valid():
            ...
            chat_completion = client.chat.completions.create(
                messages=[
                    {
                        "role": "user",
                        "content": f"{request.data.get('message')}"
                    }
                ],
            ...
            conversation=conversation_serializer.save(user=request.user,response=chat_completion.choices[0].message.content)
            
            # return the response to frontend
            return Response({"response":conversation.response},status=status.HTTP_201_CREATED)
